...
parser.add_argument('--stdev', default=0.0002, type=float, help='Smoothing noise standard deviation')
...
test_indices = torch.load('cifar_indices.pth')
testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=transform_test)
testloader = torch.utils.data.DataLoader(testset, batch_size=20, shuffle=False, num_workers=2, sampler=torch.utils.data.sampler.SubsetRandomSampler(test_indices))
...
if not os.path.exists('./epsilons'):
    os.makedirs('./epsilons')
...
def test_attack(): 
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        correct += which_correct.sum()
        total += targets.size(0)
        all_epsilons.append(epsilons)
        succeed_epsilons.append(epsilons[~which_correct])

        progress_bar(batch_idx, len(testloader), 'Acc: %.3f%% (%d/%d) | Avg epsilon: %.3f'
            % (100.*correct/total, correct, total, torch.cat(succeed_epsilons).float().mean().item()))
