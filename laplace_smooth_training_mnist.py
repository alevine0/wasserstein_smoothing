...
checkpoint_file = f'./{checkpoint_dir}/mnist_laplace_smooth_base_lr_{args.lr}_stddev_{args.stdev}_epoch_{{}}.pth'
...
def test_nominal(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            outputs = torch.log(wass_smooth_utils.laplace_smooth_forward_train(inputs, net, args.stdev))
...
if args.resume:
    ...
    checkpoint_file = './{}/mnist_laplace_smooth_base_lr_{}_stddev_{}_epoch_{}_resume_{}.pth'.format(checkpoint_dir, args.lr, args.stdev, '{}', args.resume)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        outputs_nominal = torch.log(wass_smooth_utils.laplace_smooth_forward_train(inputs, net, args.stdev))
...
def test(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        with torch.no_grad():
            ...
            outputs_nominal = torch.log(wass_smooth_utils.laplace_smooth_forward(inputs, net, args.testsamples, args.stdev))
            loss = criterion(outputs_nominal, targets)
            _, predicted_nominal = outputs_nominal.max(1)
            nominal_correct += predicted_nominal.eq(targets).sum().item()
